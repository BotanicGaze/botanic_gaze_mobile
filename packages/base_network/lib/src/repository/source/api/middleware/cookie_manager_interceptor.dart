...
class CookieManager extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    cookieJar.loadForRequest(Uri.parse(options.baseUrl)).then((cookies) {
  ...
  Future<void> _saveCookies(Response response) async {
    ...
    await cookieJar.saveFromResponse(
        Uri.parse(response.requestOptions.baseUrl), cookies);
    if (isRedirectRequest && locations.isNotEmpty) {
      final originalUri = Uri.parse(response.requestOptions.baseUrl);
      await Future.wait(
        locations.map(
