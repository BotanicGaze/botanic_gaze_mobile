...
class SpUtil {
  ...
  static List<T>? getObjList<T>(
    String key,
    T Function(Map<dynamic, dynamic> v) f, {
    List<T>? defValue = const [],
  }) {
    final dataList = getObjectList(key);
    final list = dataList?.map((value) {
      return f(value);
    }).toList();
  ...
  static List<Map<String, dynamic>>? getObjectList(String key) {
    final dataLis = _prefs?.getStringList(key);
    return dataLis?.map((value) {
      final dataMap = json.decode(value) as Map<String, dynamic>;
      return dataMap;
    }).toList();
  }
